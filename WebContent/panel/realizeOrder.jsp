...
<html>
	<head>
		...
		<title>Realizacja zamówienia - Panel pracownika</title>
	</head>
	<body>
		...
		<section class="pageContent">
			<h2>Wydawanie zamówienia</h2>
			<c:if test="${zamowienie.oplacone}">
				<p>Wydaj klientowi poniższy towar, a następnie zatwierdź operację.</p>
			</c:if>
			<c:if test="${not zamowienie.oplacone}">
				<p>Odbierz od klienta płatność, wydaj klientowi poniższy towar, a następnie zatwierdź operację.</p>
			</c:if>
			<p>Klient: ${zamowienie.klient.imie} ${zamowienie.klient.nazwisko}</p>
			<c:if test="${not zamowienie.oplacone}">
				<p>Kwota do zapłaty: ${zamowienie.totalPrice}</p>
			</c:if>
			<p>Zawartość zamówienia: 
				<c:forEach items="${zamowienie.produkty}" var="p" varStatus="i">
					<c:if test="${i.index ne 0}">, </c:if>
					${p.id.produkt.nazwa} (x${p.ilosc})
				</c:forEach>
			</p>
