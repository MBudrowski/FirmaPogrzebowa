...
<html>
	<head>
		...
		<title>Nekrologi - Firma pogrzebowa</title>
	</head>
	<body>
		...
		<section class="pageContent">
			...
			<c:forEach items="${pogrzeby}" var="p">
				<fmt:formatDate value="${p.dataPogrzebu}" var="formattedDate" type="date" pattern="yyyy-MM-dd HH:mm" />
				<p>Dnia ${p.dataSmierci} odszedł od nas <b>${p.imieDenata} ${p.nazwiskoDenata}</b> (ur. ${p.dataUrodzenia}). Wystawienie ciała nastąpi ${p.dataWystawieniaCiala} w domu pogrzebowym na ${p.domPogrzebowy.adres}.
				Nabożeństwo żałobne odbędzie się ${formattedDate} w kościele na ${p.kosciol.adres}, a następnie na cmentarzu na ${p.cmentarz.adres}.</p>
			</c:forEach>
		</section>
